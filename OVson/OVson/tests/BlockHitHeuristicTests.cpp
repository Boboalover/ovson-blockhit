...
// --- "Wait for server registration" switched off -------------------------

void test_unconfirmed_mode_is_off_by_default() {
  Detector detector;
  REQUIRE(detector.requiresServerConfirmation());
}

void test_unconfirmed_mode_fires_without_swing_or_confirmation() {
  Detector detector = enabledDetector();
  detector.setRequireServerConfirmation(false);
  // No swing recorded, no health drop, no velocity packet. Blocking with a
  // sword and taking damage is the whole test.
  const Result result = detector.observeHurt(hurt(1000));
  REQUIRE(result.playSound);
  REQUIRE(!detector.hasPendingHurt());
}

void test_confirmed_mode_still_waits() {
  Detector detector = enabledDetector();
  // Same input, default mode: nothing should fire off a bare hurt.
  const Result result = detector.observeHurt(hurt(1000));
  REQUIRE(!result.playSound);
  REQUIRE(detector.hasPendingHurt());
}

void test_unconfirmed_mode_still_rejects_environmental_damage() {
  Detector detector = enabledDetector();
  detector.setRequireServerConfirmation(false);
  // Turning confirmation off says nothing about WHERE the damage came from.
  // Fall damage while blocking is still not a block-hit.
  REQUIRE(!detector.observeHurt(hurt(1000, Hazard::Fall)).playSound);
  REQUIRE(!detector.observeHurt(hurt(2000, Hazard::Lava)).playSound);
  REQUIRE(!detector.observeHurt(hurt(3000, Hazard::Void)).playSound);
}

void test_unconfirmed_mode_still_requires_blocking_and_sword() {
  Detector detector = enabledDetector();
  detector.setRequireServerConfirmation(false);
  HurtEvent notBlocking = hurt(1000);
  notBlocking.blocking = false;
  REQUIRE(!detector.observeHurt(notBlocking).playSound);

  HurtEvent noSword = hurt(2000);
  noSword.holdingSword = false;
  REQUIRE(!detector.observeHurt(noSword).playSound);
}

void test_unconfirmed_mode_still_debounces() {
  Detector detector = enabledDetector();
  detector.setRequireServerConfirmation(false);
  REQUIRE(detector.observeHurt(hurt(1000)).playSound);
  // Inside the debounce window: no second sound, however immediate the mode.
  const Millis second = 1000 + Rules::DuplicateHurtMs;
  REQUIRE(!detector.observeHurt(hurt(second)).playSound);
  // Two independent windows have to clear, not one: the duplicate-hurt window
  // runs from the last HURT (which the suppressed one above still moved) and
  // the debounce window runs from the last TRIGGER. Waiting only for the
  // longer of the two is not enough, which is what this test caught.
  const Millis third = second + Rules::DuplicateHurtMs;
  REQUIRE(third >= 1000 + Rules::TriggerDebounceMs);
  REQUIRE(detector.observeHurt(hurt(third)).playSound);
}

void test_switching_modes_mid_session_takes_effect() {
  Detector detector = enabledDetector();
  REQUIRE(!detector.observeHurt(hurt(1000)).playSound);
  detector.setRequireServerConfirmation(false);
  REQUIRE(detector.requiresServerConfirmation() == false);
  const Result later = detector.observeHurt(hurt(1000 + Rules::TriggerDebounceMs));
  REQUIRE(later.playSound);
}
...
int main() {
  const std::vector<Test> tests = {
      ...
      {"unconfirmed_mode_is_off_by_default", test_unconfirmed_mode_is_off_by_default},
      {"unconfirmed_mode_fires_without_swing_or_confirmation", test_unconfirmed_mode_fires_without_swing_or_confirmation},
      {"confirmed_mode_still_waits", test_confirmed_mode_still_waits},
      {"unconfirmed_mode_still_rejects_environmental_damage", test_unconfirmed_mode_still_rejects_environmental_damage},
      {"unconfirmed_mode_still_requires_blocking_and_sword", test_unconfirmed_mode_still_requires_blocking_and_sword},
      {"unconfirmed_mode_still_debounces", test_unconfirmed_mode_still_debounces},
      {"switching_modes_mid_session_takes_effect", test_switching_modes_mid_session_takes_effect},
  };
