...
int main() {
  const bool hasVocabulary = OVson::NickRoll::vocabularyAvailable();
  ...
  for (const char *name : {"AmyDunn2015", "freddieburns1997", "zacharysanchez13", "XxWolfyMattxX",
                           ...
                           "ItzArchie725", "Crafty_Aidan_90", "amydunn2015",
                           "lastandmax_9",
                           "NoraFenwick2011", "SelmaHagberg1997"}) {
    expectFail(name);
  }
  if (hasVocabulary) {
    for (const char *name : {"itztimmy", "thetoxicmeow", "savagenewsub",
                             "icygirlyaxel"}) {
      expectFail(name);
    }
  }
  ...
  if (hasVocabulary) {
    check(hasTell("ItzTimmy", "stock_prefix"),
          "a stock prefix with a stock word after it");
  }
  ...
  if (hasVocabulary) {
    check(hasTell("Cute_Herr", "pool_underscore"),
          "two stock words split by an underscore");
  }
  ...
  if (!hasVocabulary) {
    std::printf("SKIP vocabulary-only checks: NickScoreVocabulary.h is not available.\n");
  }
